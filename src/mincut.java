...
public class mincut {
    ...
    static class EdmondsKarp {
        ...
        private void edmondsKarp() {
            ...
            solution.add(source);
            unsearchedNodes.add(source);
            boolean[] searched = new boolean[numOfNodes];

            while (!unsearchedNodes.isEmpty()) {
                int current = unsearchedNodes.poll();

                for(Edge edge : edgeLookup[current]) {
                    // we can still send more items to this node, meaning its part of solution (U)
                    if(!searched[edge.end] && edge.flow < edge.capacity) {
                        solution.add(edge.end);
                        unsearchedNodes.add(edge.end);
                        searched[edge.end] = true;
                    }
                }
            }
